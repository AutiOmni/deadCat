...
function indicatorColorsDown(i) {
  ...
  if ($(window).width() > 700) {
    // VOLUME CHECK
    if (
      avgVolumeCheckNum < volumeTodayCheckNum &&
      changePercentageCheckNum < 0
    ) {
  ...
  try {
    while (j < 1) {
      ...
      try {
        ...
        alert(
          "Unable locate stock ticker. Please check your input and try again!"
        );
      }
...
async function buildSearchTech(obj) {
  ...
  if (changesPercentage < 0) {
    $(".symbol-search-box").css(
      "box-shadow",
      "inset 0px 2.5px 2px var(--bear-market-color), inset 0px 2.5px 2px var(--bear-market-color)"
    );
  } else if (changesPercentage > 0) {
    $(".symbol-search-box").css(
      "box-shadow",
      "inset 0px 2.5px 2px var(--bull-market-color), inset 0px 2.5px 2px var(--bull-market-color)"
    );
  }
